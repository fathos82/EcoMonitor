...
import { SensorErrorHistoryModal } from './SensorErrorHistoryModal';
...
// ─── Mensagem de erro mais recente de um sensor (GET /sensors/{id}/errors/) ───

function SensorErrorBanner({ sensorId, sensorName }: { sensorId: number; sensorName: string }) {
  const { latestError, errors, loading } = useSensorErrors(sensorId);
  const [showHistory, setShowHistory] = useState(false);

  if (loading) {
    return (
        <div className="border-t border-red-50 px-3 py-2 flex items-center gap-2 bg-red-50/50">
          <Loader2 size={12} className="shrink-0 text-red-300 animate-spin" />
          <p className="text-[11px] text-red-300">Carregando detalhes do erro…</p>
        </div>
    );
  }
  ...
  const when = new Date(latestError.dateTime).toLocaleString('pt-BR');

  return (
      <>
        <div className="border-t border-red-50 px-3 py-2 flex items-start justify-between gap-2 bg-red-50/50">
          <div className="flex items-start gap-2 min-w-0">
            <AlertCircle size={13} className="shrink-0 text-red-400 mt-0.5" />
            <div className="min-w-0">
              <p className="text-[11px] text-red-500 leading-snug">{latestError.message}</p>
              <p className="text-[10px] text-red-300 mt-0.5">{when}</p>
            </div>
          </div>
          {errors.length > 1 && (
              <button
                  onClick={() => setShowHistory(true)}
                  className="shrink-0 text-[10px] font-bold text-red-400 hover:text-red-600 hover:bg-red-100 px-2 py-1 rounded-lg transition-colors whitespace-nowrap"
              >
                Ver histórico ({errors.length})
              </button>
          )}
        </div>

        {showHistory && (
            <SensorErrorHistoryModal
                sensorId={sensorId}
          ...
          {!loading && sensors.map(sensor => {
            ...
            return (
                <div key={sensor.id} className="bg-white rounded-xl border border-stone-100 shadow-sm">
                  ...
                  {/* Mensagem de erro detalhada (busca sob demanda, só se o sensor estiver falhando) */}
                  {!sensor.isWorking && <SensorErrorBanner sensorId={sensor.id} sensorName={sensor.name} />}
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-xl ${
                            hub.status === 'online' ? 'bg-stone-800 text-emerald-400'
                            : hub.status === 'error' ? 'bg-red-50 text-red-500'
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`hidden md:flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                            hub.status === 'online' ? 'text-emerald-500 bg-emerald-500/10'
                            : hub.status === 'error' ? 'text-red-500 bg-red-50'
                            : 'text-stone-400 bg-stone-100'
                        }`}>
                          <div className={`w-2 h-2 rounded-full bg-current ${hub.status === 'online' ? 'animate-pulse' : ''}`} />
                          {hub.status}
                        </div>
                        <div className={`md:hidden w-3 h-3 rounded-full ${
                            hub.status === 'online' ? 'bg-emerald-500'
                            : hub.status === 'error' ? 'bg-red-500'
                            : 'bg-stone-300'
                        }`} />
                        {expandedHubId === hub.id
